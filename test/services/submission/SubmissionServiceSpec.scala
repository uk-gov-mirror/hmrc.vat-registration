...
class SubmissionServiceSpec extends VatRegSpec
  ...
  "submit" should {
    "return a 200 response and successfully audit when all calls succeed" in new Setup {
      ...
      await(service.submit(vatSubmissionJson.as[JsObject], testFullSubmission, testRegId, testUserHeaders)).status mustBe OK
      verifyAudit(RegistrationSubmissionAuditModel(
        testFullSubmission,
        testRegId,
        testProviderId,
    ...
    "return a 502 response and successfully audit when submission fails with a 502" in new Setup {
      ...
      await(service.submit(vatSubmissionJson.as[JsObject], testFullSubmission, testRegId, testUserHeaders)).status mustBe BAD_GATEWAY
      verifyAudit(RegistrationSubmissionAuditModel(
        testFullSubmission,
        testRegId,
        testProviderId,
